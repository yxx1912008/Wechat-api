...
public class BotClient {
    ...
    public <T extends ApiRequest, R extends ApiResponse> void send(final T request,
        ...
        client.newCall(createRequest(request)).enqueue(new okhttp3.Callback() {
            ...
            public void onResponse(Call call, Response response) {
                try {
                    ...
                    IOException ioEx = e instanceof IOException ? (IOException) e
                        : new IOException(e);
    ...
    private OkHttpClient timeout(ApiRequest request) {
        ...
        clientWithTimeout = client.newBuilder()
            .readTimeout(timeoutMillis + 1000, TimeUnit.MILLISECONDS).build();
        return clientWithTimeout;
    }
    ...
    private Request createRequest(ApiRequest request) {
        ...
        if (Constant.GET.equalsIgnoreCase(request.getMethod())) {
            ...
            if (null != request.getParameters() && request.getParameters().size() > 0) {
                ...
                for (String key : keys) {
                    sbuf.append(key).append('=').append(request.getParameters().get(key))
                        .append('&');
                }
            ...
            for (Map.Entry<String, Object> parameter : request.getParameters().entrySet()) {
                ...
                if (value instanceof byte[]) {
                    builder.addFormDataPart(name, request.getFileName(),
                        RequestBody.create(contentType, (byte[]) value));
                } else if (value instanceof File) {
                    builder.addFormDataPart(name, request.getFileName(),
                        RequestBody.create(contentType, (File) value));
