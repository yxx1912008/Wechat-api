...
public class WeChatApiImpl implements WeChatApi {
    ...
    private static final Pattern PROCESS_LOGIN_PATTERN = Pattern
                                                           .compile("window.redirect_uri=\"(\\S+)\";");
    ...
    public void login(boolean autoLogin) {
        ...
        if (autoLogin) {
            ...
            while (logging) {
                ...
                if (null == this.uuid) {
                    ...
                    log.info("二维码获取成功："
                             + String.format("%s/qrcode/%s", Constant.BASE_URL, this.uuid));
                    this.codeUrl = String.format("%s/qrcode/%s", Constant.BASE_URL, this.uuid);
                    //  log.info("开始下载二维码");
        ...
        log.info("共有 {} 个群 | {} 个直接联系人 | {} 个特殊账号 ｜ {} 公众号或服务号", this.groupUserNames.size(),
            this.contactList.size(), this.specialUsersList.size(), this.publicUsersList.size());
    ...
    private String getUUID() {
        log.info("获取二维码UUID");
        // 登录
        ApiResponse response = this.client.send(new StringRequest(
            "https://login.weixin.qq.com/jslogin").add("appid", "wx782c26e4c19acffb").add("fun",
            "new"));
    ...
    private void getQrImage(String uuid, boolean terminalShow) {
        ...
        FileResponse fileResponse = this.client.download(new FileRequest(String.format(
            "%s/qrcode/%s", Constant.BASE_URL, uid)));
    ...
    private String checkLogin(String uuid) {
        ...
        ApiResponse response = this.client.send(new StringRequest(url).add("loginicon", true)
            .add("uuid", uid).add("tip", "1").add("_", time).add("r", (int) (-time / 1000) / 1579)
            .timeout(30));
    ...
    private boolean processLoginSession(String loginContent) {
        ...
        ApiResponse response = this.client.send(new StringRequest(loginSession.getUrl())
            .noRedirect());
        loginSession.setUrl(loginSession.getUrl().substring(0,
            loginSession.getUrl().lastIndexOf("/")));
    ...
    private void statusNotify() {
        ...
        String url = String.format("%s/webwxstatusnotify?lang=zh_CN&pass_ticket=%s", bot.session()
            .getUrl(), bot.session().getPassTicket());

        this.client.send(new JsonRequest(url).post().jsonBody()
            .add("BaseRequest", bot.session().getBaseRequest()).add("Code", 3)
    ...
    public SyncCheckRet syncCheck() {
        ...
        try {
            ApiResponse response = this.client.send(new StringRequest(url)
                .add("r", System.currentTimeMillis()).add("skey", bot.session().getSKey())
                .add("sid", bot.session().getWxSid()).add("uin", bot.session().getWxUin())
                .add("deviceid", bot.session().getDeviceId())
                .add("synckey", bot.session().getSyncKeyStr()).add("_", System.currentTimeMillis())
                .timeout(30));

            Matcher matcher = SYNC_CHECK_PATTERN.matcher(response.getRawBody());

            System.out.println("获取的返回心跳参数：\r\n" + response.getRawBody());
    ...
    public WebSyncResponse webSync() {
        String url = String.format("%s/webwxsync?sid=%s&sKey=%s&passTicket=%s", bot.session()
            .getUrl(), bot.session().getWxSid(), bot.session().getSKey(), bot.session()
            .getPassTicket());
        ...
        System.out.println("获取最新消息" + response.getRawBody());
    ...
    public void logout() {
        if (bot.isRunning()) {
            ...
            this.client.send(new StringRequest(url).add("redirect", 1).add("type", 1)
    ...
    public void loadContact(int seq) {
        log.info("开始获取联系人信息");
        while (true) {
            String url = String.format("%s/webwxgetcontact?r=%s&seq=%s&skey=%s", bot.session()
                .getUrl(), System.currentTimeMillis(), seq, bot.session().getSKey());
            ...
            List<Account> memberList = WeChatUtils.fromJson(
                WeChatUtils.toJson(jsonObject.getAsJsonArray("MemberList")),
    ...
    public void loadGroupList() {
        ...
        String url = String.format("%s/webwxbatchgetcontact?type=ex&r=%s&pass_ticket=%s", bot
            .session().getUrl(), System.currentTimeMillis() / 1000, bot.session().getPassTicket());

        // 加载群信息
        JsonResponse jsonResponse = this.client.send(new JsonRequest(url).post().jsonBody()
            .add("BaseRequest", bot.session().getBaseRequest()).add("Count", groupUserNames.size())
            .add("List", list));
    ...
    public boolean revokeMsg(String msgId, String toUser) {
        ...
        JsonResponse response = client.send(new JsonRequest(url).post().jsonBody()
            .add("SvrMsgId", msgId).add("ToUserName", toUser)
    ...
    public boolean verify(Recommend recommend) {
        String url = String.format("%s/webwxverifyuser?r=%s&lang=zh_CN&pass_ticket=%s", bot
            .session().getUrl(), System.currentTimeMillis() / 1000, bot.session().getPassTicket());
        ...
        JsonResponse response = client.send(new JsonRequest(url).post().jsonBody()
            .add("BaseRequest", bot.session().getBaseRequest()).add("Opcode", 3)
            .add("VerifyUserListSize", 1).add("VerifyUserList", verifyUserList)
            .add("VerifyContent", "").add("SceneListCount", 1).add("SceneList", Arrays.asList(33))
    ...
    public boolean addFriend(String friendUserName, String msg) {
        ...
        JsonResponse response = client.send(new JsonRequest(url).post().jsonBody()
            .add("BaseRequest", bot.session().getBaseRequest()).add("Opcode", 2)
            .add("VerifyUserListSize", 1).add("VerifyUserList", verifyUserList)
            .add("VerifyContent", msg).add("SceneListCount", 1).add("SceneList", Arrays.asList(33))
            .add("skey", bot.session().getSyncKeyStr()));
        return null != response && response.success();
    }
    ...
    public boolean createChatRoom(String topic, List<String> members) {
        String url = String
            .format("%s/webwxcreatechatroom?r=%s&lang=zh_CN", bot.session().getUrl());
        ...
        JsonResponse response = client.send(new JsonRequest(url).post().jsonBody()
            .add("MemberCount", members.size()).add("MemberList", memberList).add("Topic", topic)
            .add("BaseRequest", bot.session().getBaseRequest()));
        return null != response && response.success();
    }
    ...
    public boolean removeMemberByGroup(String member, String group) {
        ...
        JsonResponse response = client.send(new JsonRequest(url).post().jsonBody()
            .add("DelMemberList", member).add("ChatRoomName", group)
            .add("BaseRequest", bot.session().getBaseRequest()));
        return null != response && response.success();
    }
    ...
    public boolean inviteJoinGroup(String member, String group) {
        ...
        JsonResponse response = client.send(new JsonRequest(url).post().jsonBody()
            .add("AddMemberList", member).add("ChatRoomName", group)
            .add("BaseRequest", bot.session().getBaseRequest()));
        return null != response && response.success();
    }
    ...
    public boolean modifyGroupName(String oldTopic, String newTopic) {
        ...
        JsonResponse response = client.send(new JsonRequest(url).post().jsonBody()
            .add("NewTopic", newTopic).add("ChatRoomName", account.getUserName())
            .add("BaseRequest", bot.session().getBaseRequest()));
        return null != response && response.success();
    }
    ...
    private List<Account> getNameById(String id) {
        ...
        return WeChatUtils.fromJson(
            WeChatUtils.toJson(response.toJsonObject().getAsJsonObject("")),
            new TypeToken<List<Account>>() {
            });
    }
    ...
    private WeChatMessage processMsg(Message message) {
        ...
        WeChatMessage.WeChatMessageBuilder weChatMessageBuilder = WeChatMessage.builder()
            .raw(message).id(message.getId()).fromUserName(message.getFromUserName())
            .toUserName(message.getToUserName()).mineUserName(bot.session().getUserName())
            .mineNickName(bot.session().getNickName()).msgType(message.msgType()).text(content);
        ...
        if (null == fromAccount) {
            ...
            weChatMessageBuilder.fromNickName(fromAccount.getNickName()).fromRemarkName(
                fromAccount.getRemarkName());
    ...
    private String searchContent(String key, String content) {
        ...
        r = WeChatUtils.match(String.format("<%s><\\!\\[CDATA\\[(.*?)\\]\\]></%s>", key, key),
            content);
        if (StringUtils.isNotEmpty(r)) {
            return r;
        }
    ...
    private String downloadImg(String msgId) {
        return this.downloadFile(new DownLoad(ApiURL.IMAGE, bot.session().getUrl(), msgId, bot
            .session().getSKey()).msgId(msgId).saveByDay());
    }
    ...
    private String downloadIconImg(String msgId) {
        return this.downloadFile(new DownLoad(ApiURL.ICON, bot.session().getUrl(), msgId, bot
            .session().getSKey()).msgId(msgId));
    }
    ...
    private String downloadHeadImg(String userName) {
        return this.downloadFile(new DownLoad(ApiURL.HEAD_IMG, bot.session().getUrl(), userName,
            bot.session().getSKey()).msgId(userName));
    }
    ...
    private String downloadVideo(String msgId) {
        return this.downloadFile(new DownLoad(ApiURL.VIDEO, bot.session().getUrl(), msgId, bot
            .session().getSKey()).msgId(msgId).saveByDay());
    }
    ...
    private String downloadVoice(String msgId) {
        return this.downloadFile(new DownLoad(ApiURL.VOICE, bot.session().getUrl(), msgId, bot
            .session().getSKey()).msgId(msgId).saveByDay());
    }
    ...
    public MediaResponse uploadMedia(String toUser, String filePath) {
        ...
        ApiResponse response = this.client.send(new StringRequest(url).post().multipart()
            .fileName(file.getName()).add("id", "WU_FILE_0").add("name", filePath)
            .add("type", mimeType)
            ...
            .add("webwx_data_ticket", dataTicket).add("pass_ticket", bot.session().getPassTicket())
    ...
    public boolean sendImg(String toUserName, String filePath) {
        ...
        String url = String.format("%s/webwxsendmsgimg?fun=async&f=json&pass_ticket=%s", bot
            .session().getUrl(), bot.session().getPassTicket());
    ...
    public boolean sendText(String toUserName, String msg) {
        DateUtils.sendSleep();
        String url = String.format("%s/webwxsendmsg?pass_ticket=%s", bot.session().getUrl(), bot
            .session().getPassTicket());
        String msgId = System.currentTimeMillis() / 1000 + StringUtils.random(6);

        JsonResponse response = this.client.send(new JsonRequest(url)
            .post()
            .jsonBody()
            .add("BaseRequest", bot.session().getBaseRequest())
            .add("Msg",
                new SendMessage(1, msg, bot.session().getUserName(), toUserName, msgId, msgId)));
        return null != response && response.success();
    }
    ...
    public boolean sendFile(String toUser, String filePath) {
        ...
        String url = String.format("%s/webwxsendappmsg?fun=async&f=json&pass_ticket=%s", bot
            .session().getUrl(), bot.session().getPassTicket());
